...
################################# Load training data #################################
file_list=['Th0', 'Th1', 'Th2', 'Treg', 'nTh17']
bf_cropss = []
seg_cropss = []
...
################################# Pick center slice of the training images #################################
bf_slice = bf_cropss[:, 16:17, :, :] # (n_samples, 1, y, x)
seg_slice = seg_cropss[:, 16:17, :, :]

slice = np.concatenate((bf_slice, seg_slice), axis=-1)
...
viewer.add_image(np.squeeze(bf_slice[i], axis=-1), scale = np.array([1.5, 0.568, 0.568]), name='bf_slice')
viewer.add_image(np.squeeze(seg_slice[i], axis=-1), scale = np.array([1.5, 0.568, 0.568]), name='seg_slice')
...
x_train, x_test, y_train, y_test = train_test_split(slice, seg_cropss, test_size=0.1, random_state=0)
...
model = reconstruct_2d_3d(input_shape)
model_name='recons_2d_3d_v2'
...
result = model.fit(x_train, y_train, batch_size=3, epochs=30, verbose=1, validation_data = (x_test, y_test),
...
plt.savefig(path+'v2_loss_30.png')

df_history.to_csv(path+'training_history_v2.csv', index=False)
################################# Load model and retrain #################################
model_name='recons_2d_3d'
...
loaded_model.compile(loss = jaccard_loss, metrics = [jaccard_index, dice_coefficient], optimizer='adam')

result = loaded_model.fit(x_train, y_train, batch_size=3, epochs=60, verbose=1, validation_data = (x_test, y_test),
                   shuffle=True, callbacks=[stop_early, checkpointer])
...
#y_pred = loaded_model.predict(bf_slice[0:100])
y_pred = model.predict(slice[0:100])

binary_y_pred = (y_pred>0.5)
idx=55
#viewer = napari.view_image(np.squeeze(bf_slice[idx], axis=-1), blending='additive', name='imgs', scale = np.array([1.5, 0.568, 0.568]))
...
viewer.add_image(np.squeeze(bf_slice[idx], axis=-1), blending='additive', name='imgs', scale = np.array([um_per_zsice, um_per_pixel, um_per_pixel]))
#viewer.add_image(np.squeeze(y_pred[idx], axis=-1), scale = np.array([um_per_zsice, um_per_pixel, um_per_pixel]), name='prediction')
viewer.add_image(np.squeeze(binary_y_pred[idx], axis=-1), scale = np.array([um_per_zsice, um_per_pixel, um_per_pixel]), name='prediction')
viewer.add_image(np.squeeze(seg_cropss[idx], axis=-1), scale = np.array([um_per_zsice, um_per_pixel, um_per_pixel]), name='truth')
