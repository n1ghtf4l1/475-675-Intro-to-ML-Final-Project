...
from dnn.models import reconstruct_2d_3d, Attention_3DUnet
...
df_history.to_csv(path+'training_history.csv', index=False)
...
y_pred = model.predict(bf_cropss[0:100])
...
viewer.add_image(np.squeeze(bf_cropss[idx], axis=-1), blending='additive', name='imgs', scale = np.array([um_per_zsice, um_per_pixel, um_per_pixel]))
